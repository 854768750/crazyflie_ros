...
public:
        ...
        , m_startZ(0)
    {
        ...
        m_subscribeBattery = nh.subscribe("battery", 1, &Controller::batteryTest, this);
...
private:
    void batteryTest(
        const std_msgs::Float32::ConstPtr& battery)
    {
        m_battery = *battery;
	if(m_battery.data<m_battery_threshold)ROS_WARN("battery:%lf",m_battery.data);
    }
        ...
        case TakingOff:
            {
                ...
                if (transform.getOrigin().z() > m_startZ + 0.15) 
                {
                    ...
                    ROS_INFO("m_thrust:%f,Z_integral:%f",m_thrust,m_thrust / m_pidZ.ki());
                    m_state = Automatic;
                    m_thrust = 0;
                }
                else
        ...
        case Landing:
            {
                ...
                tf::StampedTransform transform;
                m_listener.lookupTransform(m_worldFrame, m_frame, ros::Time(0), transform);
                if (transform.getOrigin().z() <= m_startZ + 0.1) {
                    m_state = Idle;
                    geometry_msgs::Twist msg;
                    m_pubNav.publish(msg);
                }
        ...
        case Automatic:
            {
                tf::StampedTransform transform;
                m_listener.lookupTransform(m_worldFrame, m_frame, ros::Time(0), transform);
		//ROS_INFO("m_worldFrame:%s",m_worldFrame);
		//ROS_INFO("m_Frame:%s",m_Frame);
